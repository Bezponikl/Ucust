...
class CriticMungerSkill:
    ...
    def review_content(self, text: str, topic: str = '', target_audience: str = '') -> Dict[str, Any]:
        ...
        if not any(w in text_lower for w in ['напишите', 'переходите', 'ссылк', 'жмите', 'пишите', 'забирайте', 'промокод', 'оставляйте', 'звоните', 'заказывайте', 'делитесь', 'комментари', 'личные сообщения', 'лс', 'свяжитесь']):
            score -= 0.15
            fatal_flaws.append('Нет понятного Call to Action (призыва к действию).')
        
        # Проверка триггера вовлечения аудитории (комментарии, ЛС, реакции)
        if not any(w in text_lower for w in ['комментари', 'напишите', 'делитесь', 'обсудим', 'ответьте', 'ваше мнение', 'пишите в коммент', 'личные сообщения', 'лс', 'пишите нам', 'реакци']):
            score -= 0.1
            cautions.append('Слабый триггер вовлечения аудитории (добавьте призыв писать в ЛС или комментарии).')
