...
class SaigaLLMSkill:
    ...
    def generate_smm_post(
        ...
        if "как понимает" in topic_lower or "что умеет" in topic_lower or "простыми словами" in topic_lower or "для простых" in topic_lower or "не айтиш" in topic_lower or "понимает запрос" in topic_lower:
            lead = f"В чём разница между обычным чат-ботом и автономным ИИ-продакшном «{company_name}»?"
            body = (
                f"Сегодня базовые нейросети есть у всех. Но ручной ввод промптов и разрозненные генераторы создают новую рутину: "
                f"скопируй текст, сгенерируй фото в другом сервисе, проверь на ошибки, оформи и опубликуй вручную.\n\n"
                f"«{company_name}» объединяет копирайтинг, визуальную режиссуру и контроль качества в <b>единый бесшовный цикл</b>:\n\n"
                f"🎯 <b>1. Глубокий контекст</b> — ИИ считывает не просто слова, а специфику вашей ниши, Tone of Voice и психологию покупателя.\n"
                f"📸 <b>2. Визуальная режиссура</b> — генератор фото создаёт не случайный арт, а точный смысловой кадр с выверенной драматургией, светом и живой композицией.\n"
                f"🛡️ <b>3. Контроль качества (Pre-Mortem)</b> — встроенный критик отсекает штампы и воду до публикации.\n\n"
                f"⚡ Итог: согласованный продающий пост + кинематографичный визуал, готовые к дистрибуции в 1 клик.{visual_phrase}{comments_phrase}"
            )
            ...
            lead = f"«{company_name}»: почему будущее маркетинга — за автономными связками агентов, а не одиночными чат-ботами"
            body = (
                f"Большинство инструментов на рынке предлагают точечные решения: один пишет текст, другой генерирует картинку, "
                f"а предприниматель по-прежнему тратит часы, чтобы соединить всё воедино.\n\n"
                f"«{company_name}» — это <b>сквозная мульти-агентная экосистема</b>, которая полностью автоматизирует конвейер создания и дистрибуции контента 24/7:\n\n"
                f"🔍 <b>Аналитик</b> — парсит вводные данные, анализирует рынок и определяет ключевые точки контакта с аудиторией.\n"
                f"✍️ <b>Копирайтер</b> — формулирует структурированные публикации под особенности каждой социальной платформы.\n"
                f"🛡️ <b>Критик качества</b> — многоуровневый фильтр Tone of Voice, отсекающий клише, фальшь и воду до публикации.\n"
                f"🎬 <b>Визуальный директор</b> — генерирует живые кинематографичные кадры в связке с драматургией текста.\n"
                f"📡 <b>Дистрибьютор</b> — мгновенно форматирует и отправляет готовый результат в Telegram, VK, OK, MAX и на карты.\n\n"
                f"Реальная автономность, предсказуемое качество и полное освобождение от рутины.{comments_phrase}"
            )
            cta = f"Напишите в комментариях нишу вашего бизнеса — продемонстрируем связку агентов в действии! 🚀" if has_comments else f"Напишите нам в личные сообщения — покажем, как система масштабирует маркетинг вашего дела! 🚀"
            return {
                "post_text": f"{lead}\n\n{body}\n\n{cta}",
                "promo_code": f"{company_name.upper().replace(' ', '')}2026",
