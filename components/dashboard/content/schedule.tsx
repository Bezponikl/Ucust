...
export function MonthCalendar({
  ...
  originalDate,
  /** Подсветка периода целиком — для выбора дат «с … по …». */
  range,
}: {
  value: string;
      ...
      {picking ? (
        ...
        <div className="uc-fade-in grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: total }, (_, i) => i + 1).map((d) => {
            ...
            // Границы периода красим как выбранные, дни между ними — мягкой заливкой.
            const isEdge = Boolean(range) && (iso === range!.from || iso === range!.to);
            const inRange =
              Boolean(range) && range!.from && range!.to && iso > range!.from && iso < range!.to;
            ...
            return (
              ...
              >
                {d}
                {busy && !isSel && !isEdge && <span className="absolute bottom-1 h-1 w-1 rounded-full bg-brand" aria-hidden="true" />}
              </button>
            );
          })}
        </div>
