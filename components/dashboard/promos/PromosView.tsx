...
/** Порядок секций при группировке: сначала то, что идёт прямо сейчас. */
const STATUS_SECTIONS: PromoStatus[] = ["active", "scheduled", "finished"];

/** Карточки сделаны уже: на широком экране их помещается четыре в ряд. */
const GRID_CLASS =
  "grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 [&>*]:min-w-0";
...
function MoreMenu({
  ...
  const dir = useDropDirection(open, ref, 120);
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div
          className={`uc-pop-in absolute right-0 z-30 w-44 overflow-hidden rounded-2xl border border-border/70 p-1.5 shadow-lift ${dropClassTight(dir)} ${menuSurfaceClass(surfaceStyle)}`}
        >
          {items.map((it) => (
            <button
              key={it.id}
    ...
    >
      <div className="relative h-28 shrink-0 overflow-hidden sm:h-32">
        {p.image ? (
          ...
          <div className={`absolute inset-0 grid place-items-center bg-surface-soft bg-gradient-to-br ${t.cover}`}>
            <Icon name={t.icon} size={40} className="opacity-25" aria-hidden="true" />
          </div>
        )}
        ...
        {p.discount && (
          <span className="absolute bottom-2.5 right-3.5 font-display text-2xl font-black leading-none text-white drop-shadow-lg">
            {p.discount}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <div className="min-w-0">
          <h3 className="truncate font-display text-base font-bold leading-snug text-ink">{p.title}</h3>
      ...
      >
        ...
        <span className="hidden w-40 shrink-0 md:block">
          {pct !== null ? (
            ...
            <>
              <span className="block text-xs text-ink-muted">Цель не задана</span>
              {/* пустая дорожка держит высоту строки — столбик остаётся ровным */}
              <span className="mt-1.5 block h-1.5 rounded-full bg-border/40" aria-hidden="true" />
            </>
          )}
        </span>

        {/* Ширина фиксирована: иначе от числа иконок съезжает колонка прогресса
            и полоски в списке перестают выстраиваться в один столбик. */}
        <span className="hidden w-[4.5rem] shrink-0 justify-end lg:flex">
          <Channels ids={p.channels} />
        </span>
      </Link>
...
export default function PromosView() {
  ...
  // Группировка по статусу: карточки разложены по секциям «Активные / …»,
  // чтобы не искать глазами, что уже идёт, а что ещё только запланировано.
  ...
  const visible = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      switch (sort) {
        ...
        // Даты в ISO сортируются лексикографически — «скоро завершатся» наконец честное
        case "ending": return order[a.status] - order[b.status] || a.end.localeCompare(b.end);
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between gap-3 overflow-x-auto">
          ...
          <Segmented
            ...
            options={[
              ...
              { id: "timeline", icon: "calendar", title: "Шкала периодов" },
            ]}
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {mode !== "timeline" && (
            <button
              ...
              className={`inline-flex h-9 shrink-0 items-center gap-1.5 rounded-full border px-3.5 text-sm font-medium transition duration-200 ${
                grouped
                  ? "border-brand/40 bg-brand/8 text-brand"
                  : "border-border/70 bg-surface-soft/60 text-ink-muted hover:text-ink"
              }`}
            >
      ...
      {visible.length === 0 ? (
        ...
        <PromoTimeline promos={visible} />
      ) : grouped ? (
        <div className="flex flex-col gap-6">
          {STATUS_SECTIONS.map((s) => {
            const group = visible.filter((p) => p.status === s);
            ...
            return (
              <section key={s} className="flex flex-col gap-3">
                <h2 className="flex items-center gap-2 text-sm font-semibold text-ink">
                  ...
                  <span className="text-xs font-medium tabular-nums text-ink-muted">{group.length}</span>
                </h2>
                {mode === "grid" ? (
        ...
        <div className={`stagger-grid ${GRID_CLASS}`}>
          {visible.map((p) => (
            <PromoCard key={p.id} p={p} />
          ))}
          <NewPromoCard mode="grid" />
