...
const AI_STEPS = ["Анализируем бренд", "Определяем стиль общения", "Пишем текст публикации"];
const AI_TAIL_STEPS = ["Подбираем хэштеги", "Проверяем текст"];
const PHOTO_STEP = "Смотрим, что на фото";
/** Шаг генерации медиа — показываем только когда картинку/видео действительно рисует ИИ. */
const MEDIA_STEP = { post: "Генерируем изображение", video: "Генерируем видео" } as const;
/** Шаг ленты ожидания и хвост после последнего шага — генерация не мгновенная. */
const STEP_MS = 900;
const STEP_TAIL_MS = 400;
...
function RadioPills<T extends string>({ value, options, onChange }: { value: T; options: { id: T; label: string; icon?: IconName; disabled?: boolean; hint?: string }[]; onChange: (v: T) => void }) {
  return (
    <div className="flex flex-wrap gap-2">
      {options.map((o) => (
        <button key={o.id} type="button" onClick={() => onChange(o.id)} aria-pressed={value === o.id}
          disabled={o.disabled} title={o.hint}
          className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition ${
            o.disabled
              ? "cursor-not-allowed bg-surface-soft text-ink-muted/45"
              : value === o.id
                ? "bg-brand text-white"
                : "bg-surface-soft text-ink-muted hover:text-ink"}`}>
          {o.icon && <Icon name={o.icon} size={15} aria-hidden="true" />} {o.label}
          {o.disabled && <Icon name="lock" size={13} aria-hidden="true" />}
        </button>
      ))}
    </div>
  );
...
// canReplace=false — для видео: загрузка своего файла пока в разработке,
// поэтому кнопку «Изменить» не показываем, остаётся перегенерация.
function MediaEditor({ media, onChange, onGenerate, onUpload, onRemove, canReplace = true }: { media: Media; onChange: () => void; onGenerate: () => void; onUpload: () => void; onRemove: () => void; canReplace?: boolean }) {
  if (media.kind === "none") {
    return (
...
export default function CreateView() {
  ...
  const aiMedia = format === "video" ? vidSource === "ai" : imgSource === "ai";
  const aiSteps = [
    ...(photos.items.length > 0 ? [PHOTO_STEP] : []),
    ...AI_STEPS,
    ...(aiMedia ? [MEDIA_STEP[format]] : []),
    ...AI_TAIL_STEPS,
  ];
  ...
  /**
   * Результат показываем только после того, как лента шагов доиграла до конца:
   * иначе экран ожидания мигает и обещает готовность, которой ещё нет.
   */
  const finishWhenStepsDone = (run: () => void) => {
    const elapsed = Date.now() - startedAt.current;
    const wait = Math.max(0, aiSteps.length * STEP_MS + STEP_TAIL_MS - elapsed);
    if (finishTimer.current) clearTimeout(finishTimer.current);
    finishTimer.current = setTimeout(run, wait);
  };
  ...
  const runGeneration = () => {
    ...
    const fallback = (note?: string) => {
      if (note) toast(note);
      finishWhenStepsDone(() => finishGeneration(generateBody(topic, format, photos.items.length), null));
    };
    ...
    void (async () => {
      try {
        ...
        finishWhenStepsDone(() => finishGeneration(body, postId));
      } catch (err) {
                  ...
                  {format === "post" ? (
                    ...
                    <>
                      <RadioPills value={vidSource} onChange={(v) => { setVidSource(v); if (v === "ai") aiVideo(); else videoInput.current?.click(); }}
                        options={[
                          { id: "upload", label: "Загрузить своё видео", disabled: true, hint: "Пока в разработке" },
                          { id: "ai", label: "Сгенерировать AI" },
                        ]} />
                      <p className="mt-2 text-xs text-ink-muted">Загрузка своего видео пока в разработке — доступна генерация AI</p>
                    </>
                  )}
                </div>
          ...
          <div className="uc-fade-in flex flex-col gap-4 rounded-[28px] border border-border bg-card p-5 sm:p-6 lg:min-h-0 lg:flex-1">
            ...
            {(format === "post" || media.kind !== "none") && (
              <div className="lg:min-h-[220px] lg:flex-[1.3]">
                <MediaEditor media={media} canReplace={format !== "video"} onChange={() => photoInput.current?.click()} onUpload={() => photoInput.current?.click()} onGenerate={() => (format === "video" ? aiVideo() : aiImage())} onRemove={() => setMedia({ kind: "none" })} />
              </div>
            )}
          ...
          <div className="flex min-h-[540px] flex-col rounded-[28px] border border-border bg-card p-6 sm:p-7 lg:min-h-0 lg:flex-1">
            {mode === "generating" ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-6 py-10 text-center">
                ...
                <div><p className="text-lg font-bold text-ink">UCust генерирует публикацию</p><p className="mt-1 text-sm text-ink-muted">Обычно 20–60 секунд — не закрывайте вкладку</p></div>
