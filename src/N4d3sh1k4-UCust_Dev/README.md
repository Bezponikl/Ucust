...
- **configuration-service** — Spring Cloud Config Server (в текущей версии не
  используется, каждый сервис имеет локальный `application.yml`).
...
- **Exchange:** `user-exchange` (Topic)
- **Routing keys:** `user.registration.email`, `user.password.reset`,
  `user.account.locked`
...
├── ops/                # Инфраструктура (PostgreSQL, RabbitMQ, MinIO)
