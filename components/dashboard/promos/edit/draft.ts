...
export function draftFromPromo(p: Promo): PromoDraft {
  // Даты у акции уже в ISO — разбирать подпись периода больше не нужно
  // (и нельзя: «20 дек — 10 янв» разъезжается по годам).
  const dateFrom = p.start || isoOffset(0);
  const dateTo = p.end || isoOffset(14);
