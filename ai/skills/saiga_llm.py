...
class SaigaLLMSkill:
    ...
    def _sanitize_llm_post(text: str, company_name: str) -> str:
        ...
        - Преобразует дефисные списки (- **35% ...**) в чистый естественный текст
        - Удаляет спам эмодзи-цифрами (1️⃣, 2️⃣, 3️⃣) и клише.
        """
        if not text:
            return ""
        ...
        for line in text.splitlines():
            ...
            # Удаляем служебные мета-заголовки структуры промпта
            ...
            s = re.sub(r'!{2,}', '!', s)
            # Убираем звездочки Markdown (**текст** -> текст)
            s = re.sub(r'\*\*(.*?)\*\*', r'\1', s)
            s = s.replace('**', '').replace('*', '')
            # Преобразуем списочные дефисы в начале строки (- текст -> • текст)
            s = re.sub(r'^\s*[-–—]\s+', '• ', s)
            # Нормализация тире в середине строки
            s = re.sub(r'\s+[—–]\s+', ' - ', s)
            lines.append(s)
