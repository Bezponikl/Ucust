...
# ============================================================================
# ТАРИФЫ И КВОТИРОВАНИЕ ПОДПИСОК (TARIFFS & SUBSCRIPTION QUOTAS)
# ============================================================================
...
async def get_tariffs():
    """
    💳 ПОЛНАЯ ТАРИФНАЯ СЕТКА И МАТРИЦА ВОЗМОЖНОСТЕЙ AI-КОНТУРА:
    - Возвращает все тарифные планы (START, BUSINESS, ENTERPRISE, CUSTOM).
    - Лимиты генераций в месяц, допустимые дни недели и расписание.
    - Матрица поддерживаемых фичей (VLM, OCR, Clean RAG, ComfyUI, видео LTX-2.3, каналы).
    """
    return {
        "status": "success",
...
async def get_client_subscription_quota(
    client_id: str = Query("default_client", description="ID клиента / проекта"),
    tier: Optional[str] = Query(None, description="Опциональный тариф для расчета (START, BUSINESS, ENTERPRISE, CUSTOM)")
):
    """
    📊 РАСЧЕТ И ПРОВЕРКА КВОТЫ ПОДПИСКИ КЛИЕНТА:
    - Возвращает лимиты выбранного тарифа, статус активности и остаток квоты.
    - Автоматически рассчитывает календарную сетку слотов для постинга на 30 дней вперед.
    """
    tier_key = (tier or "BUSINESS").upper()
    tier_info = PLATFORM_TARIFFS.get(tier_key, PLATFORM_TARIFFS["BUSINESS"])

    # Рассчитываем 30-дневное календарное расписание
