...
class SaigaLLMSkill:
    ...
    def generate_smm_post(
        ...
        if "как понимает" in topic_lower or "что умеет" in topic_lower or "простыми словами" in topic_lower or "для простых" in topic_lower or "не айтиш" in topic_lower or "понимает запрос" in topic_lower:
            lead = f"Как нейросеть понимает ваши мысли и создает живые фото для бизнеса? Простыми словами"
            body = (
                f"Вам больше не нужно разбираться в заумных IT-терминах, составлять километровые ТЗ или часами подбирать референсы.\n\n"
                f"В «{company_name}» вы общаетесь с ИИ-агентами на простом человеческом языке:\n\n"
                f"💡 <b>1. Вы говорите обычными словами:</b>\n"
                f"<i>«Нужно уютное фото капучино на деревянном столике у окна для утреннего поста»</i>.\n\n"
                f"🧠 <b>2. Как нейросеть считывает ваш запрос:</b>\n"
                f"• Мгновенно понимает контекст, настроение и специфику вашей ниши.\n"
                f"• Сама подбирает естественный утренний свет из окна, глубину резкости и живые текстуры — без глянца и пластика.\n"
                f"• Результат выглядит как кадр, снятый вашим клиентом — живой, честный и вызывающий доверие.\n\n"
