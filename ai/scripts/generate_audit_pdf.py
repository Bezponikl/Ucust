...
def build_pdf():
    ...
    super_features = [
        ...
        ("9. Замкнутый цикл обратной связи и расчет позитивности (FeedbackLoopEngine)",
         "Система оценивает качество восприятия постов по 3 математическим формулам: Индекс чистого одобрения (NAI = L / (L + D + 1)), Взвешенная позитивность WPR% с повышенным весом комментариев (1.5x) и дизлайков (-2.0x), а также Логарифмический масштабированный балл (Score = log10(V + 1) * ((1.0*L + 1.5*R + 1) / (1.0*D + 1))). Извлекает возражения и вопросы покупателей для авто-адаптации контент-плана."),
