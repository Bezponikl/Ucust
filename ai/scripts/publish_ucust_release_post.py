# -*- coding: utf-8 -*-
"""
Публикация медиа-поста с 4 фото (MediaGroup) и текстом с новыми фичами Telegram 7.2+ в @testaipublisher.
"""
...
# Пути к 4 загруженным изображениям
IMAGES = [
...
POST_TEXT = """Пока индустрия привыкала к картинкам с шестью пальцами на руках и лицам из жидкого пластика, мы в <b>UCust</b> тихо готовили революцию. Мы активно приближаемся к релизу и сегодня хотим показать, чему научили нашего автономного агента-визуализатора.

<tg-spoiler>Спойлер: стоковые фотобанки уже нервно курят в сторонке.</tg-spoiler>

Все 4 кадра в карусели сгенерированы полностью с нуля — без фотошопа, магии вне Хогвартса и ручной ретуши.
...
async def publish_album_and_post():
    url = f"https://api.telegram.org/bot{BOT_TOKEN}"
    
    # 1. Отправляем альбом из 4 фото (sendMediaGroup)
    media_items = []
    files_to_open = {}
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            print("📸 Отправка альбома из 4 изображений...")
            data = {
                "chat_id": TARGET_CHANNEL,
                "media": json.dumps(media_items)
            }
            resp_album = await client.post(f"{url}/sendMediaGroup", data=data, files=files_to_open)
            print(f"Статус отправки альбома: {resp_album.status_code}")
            
            # 2. Отправляем богатый интерактивный текст с кнопками (sendMessage)
            print("📝 Отправка интерактивного текста с expandable_blockquote, спойлером и кнопками...")
            msg_payload = {
                "chat_id": TARGET_CHANNEL,
                "text": POST_TEXT,
                "parse_mode": "HTML",
            ...
            res_json = resp_text.json()
            if resp_text.status_code == 200 and res_json.get("ok"):
                print(f"✅ Пост успешно опубликован в {TARGET_CHANNEL}! Message ID: {res_json['result']['message_id']}")
            else:
                print(f"❌ Ошибка отправки текста: {res_json}")
    finally:
...
if __name__ == "__main__":
    asyncio.run(publish_album_and_post())
