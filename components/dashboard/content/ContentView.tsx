...
/**
 * Кнопка-фильтр контент-плана. Раньше это была почти невидимая плашка: на белой
 * панели она читалась как обычный текст. Теперь у неё явная рамка, подложка и
 * подсветка в раскрытом состоянии — видно, что на неё можно нажать.
 */
function filterTriggerClass(open: boolean): string {
  return `inline-flex min-h-9 items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-sm font-semibold transition ${
    open
      ? "border-brand bg-brand/8 text-brand ring-1 ring-brand/25"
      : "border-border bg-surface-soft text-ink hover:border-brand/50 hover:bg-brand/5"
  }`;
}
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-3 shadow-soft sm:flex-row sm:items-center sm:gap-x-3 sm:p-4 lg:gap-x-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:justify-start">
        <span className="shrink-0 text-sm font-medium text-ink-muted">Социальные сети</span>
      ...
      <div className="flex items-center justify-between gap-2 sm:justify-start">
        <span className="shrink-0 text-sm font-medium text-ink-muted">Тип</span>
        <div ref={typeAnchor} className="relative">
          <button
            type="button"
            onClick={() => setOpenType((v) => !v)}
            aria-expanded={openType}
            className={filterTriggerClass(openType)}
          >
            {typeLabel}
      ...
      {/* Статус — дропдаун */}
      <div className="flex items-center justify-between gap-2 sm:justify-start">
        <span className="shrink-0 text-sm font-medium text-ink-muted">Статус</span>
        <div ref={statusAnchor} className="relative">
          <button
            type="button"
            onClick={() => setOpenStatus((v) => !v)}
            aria-expanded={openStatus}
            className={filterTriggerClass(openStatus)}
          >
