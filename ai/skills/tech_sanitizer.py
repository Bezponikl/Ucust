...
class TechSanitizer:
    ...
    REPLACEMENTS: List[Tuple[Pattern, str]] = [
        ...
        (re.compile(r'\b(qwen(?:-image|-vl)?|квен)\b', re.IGNORECASE), "нейрофотограф"),
        (re.compile(r'\b(lora|лора)\b', re.IGNORECASE), "стилевой фильтр"),
        (re.compile(r'\b(stable\s*diffusion|sdxl|сдхл)\b', re.IGNORECASE), "диффузионный фотогенератор"),
