...
const OAUTH_ERRORS: Record<string, string> = {
  email_not_found: "Яндекс не поделился адресом почты — разрешите доступ или войдите по паролю",
...
export function oauthErrorMessage(code: string | null): string | null {
  if (!code) return null;
  return OAUTH_ERRORS[code] ?? "Не удалось войти через Яндекс — попробуйте ещё раз";
}
