...
import { dropClassTight, useDropDirection } from "@/lib/useDropDirection";
...
interface DetailProps {
  ...
  draftValue: string;
  sentReply?: string;
  /** Включён ли ИИ-автоответ: от этого зависит подача формы, но не её наличие. */
  autoReply: boolean;
  /** Сколько сообщений диалога учтено в черновике. */
  contextUsed: number;
...
function DetailPanel({ item, isAnswered, draftValue, sentReply, autoReply, contextUsed, onDraftChange, onSend, onRefresh, onBack }: DetailProps) {
  return (
    <div className="flex h-full flex-col">
      ...
      {/* Форма ответа есть всегда: выключенный ИИ убирает подсказку, а не
          возможность ответить. С включённым — поле приходит с черновиком ИИ. */}
      {!isAnswered && (
        <div className="shrink-0 border-t border-border/40 px-4 py-3">
          <div className={`rounded-2xl border p-3.5 ${autoReply ? "border-brand/20 bg-brand/5" : "border-border bg-surface-soft/60"}`}>
            <span className={`mb-2 flex flex-wrap items-center gap-1.5 text-xs font-semibold ${autoReply ? "text-brand" : "text-ink-muted"}`}>
              {autoReply ? (
                <>
                  <Icon name="sparkles" size={13} aria-hidden="true" /> ИИ предлагает ответ
                  {contextUsed > 1 && (
                    <span className="font-medium text-ink-muted">
                      · учтено сообщений в диалоге: {contextUsed}
                    </span>
                  )}
                </>
              ) : (
                <>
                  <Icon name="send" size={13} aria-hidden="true" /> Ваш ответ
                </>
              )}
            </span>
            <textarea
              ...
              placeholder={autoReply ? undefined : "Напишите ответ клиенту"}
              className="min-h-20 w-full resize-none rounded-xl border border-border bg-card px-3 py-2 text-[0.8125rem] leading-relaxed text-ink outline-none transition placeholder:text-ink-muted focus:border-brand focus:ring-1 focus:ring-brand/25"
            />
            <div className="mt-2.5 flex flex-wrap gap-2">
              <button type="button" onClick={onSend} disabled={!draftValue.trim()}
                className="btn-glass-blue inline-flex flex-1 items-center justify-center gap-2 px-4 py-2 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50">
              ...
              <button type="button" onClick={onRefresh}
                ...
                <Icon name={autoReply ? "refresh" : "sparkles"} size={14} aria-hidden="true" />
                {autoReply ? "Другой вариант" : "Предложить ИИ"}
              </button>
            </div>
          </div>
        </div>
...
export default function InboxView() {
  ...
  const filterDir = useDropDirection(filterMenuOpen, filterMenuRef, 320);
  ...
  // Черновик считаем по всей переписке (buildAiDraft), а не по последней реплике.
  // С выключенным ИИ поле стартует пустым — подсказку можно запросить кнопкой.
  const draftOf = (i: InboxItem) => drafts[i.id] ?? (autoReply ? buildAiDraft(i).text : "");
        ...
        <div className={`flex w-full shrink-0 flex-col border-r border-border/40 lg:w-80 xl:w-96 ${selected ? "hidden lg:flex" : "flex"}`}>
          ...
          {!isMeta && (
            <div ref={filterMenuRef} className="relative shrink-0 border-t border-border/40 px-3 py-2.5">
              ...
              {filterMenuOpen && (
                <div className={`absolute left-3 right-3 z-30 max-h-[70vh] overflow-y-auto rounded-2xl border border-border p-4 shadow-lift ${dropClassTight(filterDir)} ${menuSurfaceClass(surfaceStyle)}`}>
            ...
            {list.map((i) => {
              ...
              return (
                <li key={i.id} className="group relative">
                  <SwipeToDelete onDelete={() => deleteItem(i.id)} label={`Удалить обращение от ${i.author}`}>
                  ...
                  {/* Кнопка удаления — для мыши. На тач-экранах её заменяет
                      свайп влево: навести палец на :hover невозможно. */}
                  <button type="button" onClick={(e) => { e.stopPropagation(); deleteItem(i.id); }}
                    aria-label={`Удалить обращение от ${i.author}`}
                    className="absolute right-1.5 top-1.5 hidden h-6 w-6 items-center justify-center rounded-full text-ink-muted opacity-0 transition hover:bg-red-500/10 hover:text-red-500 group-hover:opacity-100 lg:flex">
        ...
        <div className={`flex min-w-0 flex-1 flex-col ${selected ? "flex" : "hidden lg:flex"}`}>
          {!selected ? (
            ...
            <DetailPanel
              ...
              autoReply={autoReply}
              contextUsed={threadContext(selected).length}
              onRefresh={() => setDrafts((d) => ({ ...d, [selected.id]: buildAiDraft(selected).text }))}
