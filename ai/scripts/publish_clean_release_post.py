# -*- coding: utf-8 -*-
"""
Чистая публикация релиза UCust:
1. Альбом из 4 фото (ровно 1 раз) с подписью.
2. Подробный текст с expandable_blockquote, спойлером, <code> почты и интерактивными кнопками.
"""
...
ALBUM_CAPTION = """📸 <b>Карусель генераций: Автономный визуализатор UCust</b>
<tg-spoiler>Все 4 кадра созданы нейросетью с нуля без ручной ретуши</tg-spoiler>"""
...
Все 4 кадра выше сгенерированы полностью с нуля — без фотошопа, магии вне Хогвартса и ручной ретуши.
...
async def main():
    ...
    # 1. Сначала удаляем все предыдущие тестовые сообщения, чтобы канал был идеально чистым
    async with httpx.AsyncClient(timeout=30.0) as client:
        print("🧹 Очистка старых сообщений в канале...")
        for m_id in range(260, 300):
            try:
                await client.post(f"{url}/deleteMessage", json={"chat_id": TARGET_CHANNEL, "message_id": m_id})
            except Exception:
                pass
                
        print("📸 Отправка альбома из 4 кадров...")
        media_items = []
        files_to_open = {}
        ...
        print("📝 Отправка интерактивного поста с кнопками...")
        msg_payload = {
            "chat_id": TARGET_CHANNEL,
            "text": POST_TEXT,
            "parse_mode": "HTML",
        ...
        print(f"Текст: {resp_text.status_code}, ok: {res_j.get('ok')}, id: {res_j.get('result', {}).get('message_id')}")
